...
from imitation_cl.data.lasa import LASA, LASAExtended, LASAExtendedPseudo
...
def parse_args(return_parser=False):
    ...
    parser.add_argument('--data_type', type=str, default='np', help='Type of data to load from - mat: mat files, np: numpy archives')
...
def gen_pseudo_datasets(args, save_dir, train_task_id, start_positions, timesteps, device):

    last_train_task_id = train_task_id - 1

    # Load the networks for the current task_id
    tnet = torch.load(os.path.join(save_dir, 'models', f'tnet_{last_train_task_id}.pth'))
    node = torch.load(os.path.join(save_dir, 'models', f'node_{last_train_task_id}.pth'))
    ...
    pseudo_datasets = list()

    # Generate data for all tasks that this model has been trained on
    for eval_task_id in range(last_train_task_id+1):
        ...
        y_start = start_positions[eval_task_id]
        ...
        if args.data_class == 'LASA':
            pseudo_dataset = LASAExtendedPseudo(data=y_hat_np, timesteps=timesteps.cpu().detach().numpy(), seq_len=args.tsub, norm=True, device=device)
            y_hat_np = pseudo_dataset.unnormalize(y_hat_np)
            pseudo_dataset = LASAExtendedPseudo(data=y_hat_np, timesteps=timesteps.cpu().detach().numpy(), seq_len=args.tsub, norm=True, device=device)
        ...
        pseudo_datasets.append(pseudo_dataset)

        print(y_hat_np.shape)
        fig, ax = plt.subplots(1,2, sharex=True, sharey=True)
        for a in ax:
            a.set_aspect('equal')
        ax[0].scatter(y_hat_np[:,:,0], y_hat_np[:,:,1], s=0.1)

        filenames = get_sequence(args.seq_file)  
        datafile = os.path.join(args.data_dir, filenames[eval_task_id])
        orig_data = LASAExtended(datafile, seq_len=args.tsub, norm=True, device=device)
        
        y_all_np = orig_data.pos.cpu().detach().numpy()
        y_all_np = orig_data.unnormalize(y_all_np)
        ax[1].scatter(y_all_np[:,:,0], y_all_np[:,:,1], s=0.1)

        plt.savefig(f't{train_task_id}_e{eval_task_id}.png', bbox_inches='tight')

    return pseudo_datasets


def train_task(args, task_id, tnet, node, start_positions, timesteps, save_dir, device):
    ...
    # Start position of the current task
    ...
    if task_id == 0:
        ...
        # Generate pseudo datasets for all previous tasks (0,...,task_id-1)
        datasets = gen_pseudo_datasets(args, save_dir, task_id, start_positions, timesteps, device)
        print(f'Pseudo datasets count: {len(datasets)}')

        # Append the start of the current task to the list of start positions
        start_positions.append(start_pos)

        # Append the real data for the current task to the pseudo datasets of previous tasks
        datasets.append(data)
        print(f'All datasets count: {len(datasets)}')
    ...
    # Create a new task embedding for the current task
    ...
    return tnet, node, start_positions, timesteps
...
def train_all(args):
    ...
    eval_resuts=None
    start_positions, timesteps = None, None
    ...
    for task_id in range(num_tasks):
        ...
        tnet, node, start_positions, timesteps = train_task(args, task_id, tnet, node, start_positions, timesteps, save_dir, device)
