...
from imitation_cl.train.utils import check_cuda, set_seed, get_sequence, get_beta_for_tasks
from imitation_cl.model.hypernetwork import ChunkedHyperNetwork, ChunkedTaskEmbHypernetwork, str_to_ints, get_current_targets, calc_delta_theta, calc_fix_target_reg_stochastic
from imitation_cl.model.node import NODE
from imitation_cl.model.lsddm_hn import configure as configure
from imitation_cl.model.lsddm_hn_t import configure as configure_t
...
from imitation_cl.logging.utils import custom_logging_setup, write_dict, read_dict
...
def parse_args(return_parser=False):
    ...
    parser.add_argument('--num_regs', type=int, required=True, help='Number of past tasks used for regularization')
...
def train_task(args, task_id, hnet, tnet, node, beta, device, param_names, pbar=trange, writer=None, save_dir=""):
    ...
    for iteration in trange(args.num_iter):
        ...
        if calc_reg:
            ...
            loss_reg = calc_fix_target_reg_stochastic(hnet, 
                                                      task_id,
...
def eval_task(args, eval_task_id, hnet, tnet, node, device, param_names, train_task_id, writer=None):
    ...
    return eval_traj_metrics, eval_traj_metric_errors, plot_data, node
...
def eval_during_train(args, save_dir, train_task_id, eval_results=None, writer=None):
    ...
    for eval_task_id in range(train_task_id+1):
        ...
        eval_traj_metrics, eval_traj_metric_errors, plot_data, node = eval_task(args, eval_task_id, hnet, target_network, node, device, param_names, train_task_id, writer)
