...
from imitation_cl.metrics.entropy import batch_entropy
...
def calc_fix_target_reg_stochastic(hnet, 
                                   task_id, 
                                   num_regs,
                                   targets=None, 
                                   dTheta=None, 
    ...
    task embeddings. Instead of considering the task embeddings of all previous
    tasks, a fixed number of task embeddings are sampled and used for regularization
    in each iteration.
    ...
    Args:
        ...
        task_id: The ID of the current task (the one that is used to
            compute dTheta.
        num_regs: How many past task embs are to be sampled for regularization.
    ...
    num_past_tasks = task_id

    # List of all past task ids
    all_ids = list(range(task_id))

    # Check if num_regs is more than the number of previous tasks
    if num_regs >= num_past_tasks:
        # If num_regs is more than/equal to the number of previous tasks
        # Use all the past task_ids for regularization
        ids_to_reg = all_ids
    else:
        # If num_regs is less than the number of previous tasks
        # Uniformly sample `num_regs` previous task_ids from [0, 1, ..., task_id-1]
        ids_to_reg = np.random.choice(all_ids, size=num_regs, replace=False)
...
class HyperNetwork(nn.Module):
    ...
    def init_batch_entropy_new_task(self):
        ...
        self.compute_batch_entropy = False # Changed to false on 07.04.2023
...
class ChunkedTaskEmbHypernetwork(HyperNetwork):
    ...
    def gen_new_task_emb(self) -> None :
        ...
        chunked_task_emb = nn.Parameter(data=torch.randn((self.num_chunks, self.te_dim)), requires_grad=True)
        assert not(torch.isnan(chunked_task_emb).any()), f'nans in TE {torch.isnan(chunked_task_emb).any()}'
