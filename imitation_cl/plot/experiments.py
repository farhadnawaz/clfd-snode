...
def create_results_df(base_logdir, insert_comment='', verbose=False):
    ...
    for e in eval_paths:
        ...
        # Patch for stochastic regularization
        # Only CHN/HN with stochastic regularization will have this argument
        stoch_reg_num = None
        if 'num_regs' in args_dict.keys():
            stoch_reg_num = args_dict['num_regs']
        else:
            if '_chn_' in script_name.lower() or '_hn_' in script_name.lower():
                stoch_reg_num = 'all'
            else:
                stoch_reg_num = 'na'
        ...
        for train_task in metric_errors.keys():
            ...
            for eval_task in metric_errors[train_task].keys():
                ...
                try:
                    # If there is an error here, then we are dealing with only position data
