...
from imitation_cl.data.robottasks import RobotTasksPosition, RobotTasksOrientation, RobotTasksPositionOrientation
...
def train_task(args, task_id, tnet, node, device):
    ...
    for t in range(task_id+1):
        ...
        if args.data_class == 'LASA':
            ...
            if args.data_type == 'np':
                data = RobotTasksPositionOrientation(data_dir=args.data_dir, datafile=filenames[t], device=device, scale=args.tangent_vec_scale)
            else:
                raise NotImplementedError(f'data_type {args.data_type} not available for data_class {args.data_class}')
        else:
    ...
    for training_iters in trange(replay_train_iters):
        ...
        if loss.item() <= best_loss:
            ...
            best_tnet = deepcopy(tnet)
            best_loss = loss.item()
            best_iter = int(training_iters)        

    return best_tnet, best_node, best_loss, best_iter
...
def train_all(args):
    ...
    # one for each task
    node = NODETaskEmbedding(tnet, args.task_emb_dim, args.explicit_time, method=args.int_method, verbose=True).to(device)
    ...
    for task_id in range(num_tasks):
        ...
        tnet, node, best_loss, best_iter = train_task(args, task_id, tnet, node, device)
