...
from imitation_cl.model.hypernetwork import ChunkedHyperNetwork, ChunkedTaskEmbHypernetwork, TargetNetwork, str_to_ints, str_to_act, get_current_targets, calc_delta_theta, calc_fix_target_reg
...
def eval_task(args, task_id, hnet, tnet, node, device, writer=None):
    ...
    if args.zero_center == 1:
        plot_data = {'t': t.detach().cpu().numpy(),
                    'y_all': data.pos_goal_origin.cpu().detach().numpy(),
